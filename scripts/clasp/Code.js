...
function handleRequest_(e) {
  ...
  // 2. Route Action — merge query params into payload for GET support
  const action = body.action || (e.parameter && e.parameter.action);
  const cabangId = body.cabangId || (e.parameter && e.parameter.cabangId);
  const merged = Object.assign({}, body.payload || {}, e.parameter || {});
  // Auto-parse JSON-stringified values from query params
  const payload = {};
  Object.entries(merged).forEach(([k, v]) => {
    if (k === 'x-api-key' || k === 'action' || k === 'cabangId') return;
    if (typeof v === 'string' && v.length > 1 && ((v[0] === '{' && v[v.length-1] === '}') || (v[0] === '[' && v[v.length-1] === ']'))) {
      try { payload[k] = JSON.parse(v); return; } catch(e) {}
    }
    payload[k] = v;
  });
